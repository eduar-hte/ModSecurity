...
std::unique_ptr<std::string> LMDB::resolveFirst(std::string_view var) {
    int rc;
    MDB_val mdb_key;
    MDB_val mdb_value;
...
void LMDB::setExpiry(std::string_view key, int32_t expiry_seconds) {
    int rc;
    MDB_txn *txn;
    MDB_val mdb_key;
    MDB_val mdb_value;
...
void LMDB::delIfExpired(std::string_view key) {
    MDB_txn *txn;
    MDB_val mdb_key;
    MDB_val mdb_value_ret;
    CollectionData collectionData;
...
bool LMDB::storeOrUpdateFirst(std::string_view key,
...
void LMDB::resolveSingleMatch(std::string_view var,
...
bool LMDB::updateFirst(std::string_view key,
    std::string_view value) {
    int rc;
    MDB_txn *txn;
    MDB_val mdb_key;
...
void LMDB::del(std::string_view key) {
    int rc;
    MDB_txn *txn;
    MDB_val mdb_key;
    MDB_val mdb_value;
...
void LMDB::resolveMultiMatches(std::string_view var,
    std::vector<const VariableValue *> &l,
...
void LMDB::resolveRegularExpression(std::string_view var,
    std::vector<const VariableValue *> &l,
    variables::KeyExclusions &ke) {
    MDB_val key, data;
    MDB_txn *txn = NULL;
